...
public class GuestbookPersistenceTest {
	...
	protected OrderByComparator<Guestbook> getOrderByComparator() {
		return OrderByComparatorFactoryUtil.create(
			...
			"statusByUserName", true, "statusDate", true, "name", true);
	}
	...
	public void testResetOriginalValues() throws Exception {
		...
		Assert.assertTrue(
			Objects.equals(
				existingGuestbook.getUuid(),
				ReflectionTestUtil.invoke(
					existingGuestbook, "getOriginalUuid", new Class<?>[0])));
		Assert.assertEquals(
			Long.valueOf(existingGuestbook.getGroupId()),
			ReflectionTestUtil.<Long>invoke(
				existingGuestbook, "getOriginalGroupId", new Class<?>[0]));
	}
